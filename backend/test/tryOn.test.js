...
test('configuration requires Hugging Face credentials and a separate access code', () => {
  ...
  assert.equal(getTryOnConfig(env).model, 'black-forest-labs/FLUX.2-klein-4B');
...
test('generation routes through Hugging Face without logging credentials or photos', async () => {
  ...
  try {
    ...
    assert.match(request.url, /router\.huggingface\.co\/fal-ai\/models\/black-forest-labs\/FLUX\.2-klein-4B/);
